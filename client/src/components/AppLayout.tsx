...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  return (
    <div className="ns-app ns-noise flex min-h-screen flex-col">
      ...
      <header className="ns-topbar ns-surface sticky top-0 z-50 border-x-0 border-t-0">
        <div className="container flex h-[4.35rem] items-center justify-between gap-3">
          <Link href="/" className="group flex shrink-0 items-center gap-2.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500">
            ...
            <div className="hidden sm:block">
              <span className="ns-brand-title block font-['Sora'] text-base font-bold tracking-tight">NorthStar</span>
              <span className="block text-[10px] font-semibold uppercase tracking-[0.16em] text-sky-700">Resource graph</span>
            </div>
          </Link>

          <nav aria-label="Primary" className="ns-nav-panel hidden items-center rounded-2xl border p-1 md:flex">
            {primaryNavigation.map((item) => {
              const Icon = item.icon;
              return (
                <Link key={item.href} href={item.href} className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold transition-all duration-200 ${isNavigatorRouteActive(location, item.href) ? "ns-nav-link-active" : "ns-nav-link"}`}>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <Button variant="outline" onClick={() => setCommandOpen(true)} className="ns-search-trigger ns-control-button hidden h-9 rounded-xl px-2.5 sm:inline-flex">
            ...
            <Button variant="ghost" size="sm" className="ns-control-button h-9 gap-1.5 px-2 text-xs font-semibold" onClick={toggleLocale} aria-label={locale === "en" ? "Switch to Arabic" : "Switch to English"}><Languages className="h-4 w-4" />{locale === "en" ? "ع" : "EN"}</Button>
            <Button variant="ghost" size="icon" className="ns-control-button h-9 w-9 rounded-xl" onClick={toggleTheme} aria-label={theme === "light" ? t("switchToDark") : t("switchToLight")}>
            ...
            {isAuthenticated && user ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="sm" className="ns-control-button h-9 gap-2 rounded-xl px-1.5 pr-2.5">
            ...
            ) : <Link href="/welcome"><Button size="sm" className="ns-primary-button h-9 rounded-xl px-3.5">{t("signIn")}</Button></Link>}
        ...
        {mobileMenuOpen && <div className="ns-mobile-panel border-t shadow-[0_16px_30px_rgba(15,23,42,0.08)] md:hidden"><nav aria-label="Mobile navigation" className="container grid gap-1 py-3">{primaryNavigation.map((item) => { const Icon = item.icon; return <Link key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold ${isNavigatorRouteActive(location, item.href) ? "bg-sky-50 text-sky-800" : "ns-nav-link"}`}><Icon className="h-4 w-4" />{item.label}</Link>; })}{isAuthenticated && <Link href="/submit" onClick={() => setMobileMenuOpen(false)} className="ns-primary-button mt-1 flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold"><Plus className="h-4 w-4" />Submit a resource</Link>}</nav></div>}
      </header>
      ...
      <footer className="ns-footer mt-16 border-t">
