...
export function ReportResourceDialog({ resourceId, isAuthenticated }: { resourceId: number; isAuthenticated: boolean }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline" onClick={openReport}><Flag className="mr-2 h-4 w-4" /> Report</Button></DialogTrigger><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>Report a resource concern</DialogTitle><DialogDescription>Your report is sent to moderators for review. It does not automatically alter this resource.</DialogDescription></DialogHeader><form className="space-y-4" onSubmit={(event) => { event.preventDefault(); report.mutate({ resourceId, reason, details: details.trim() || undefined }); }}><label className="block space-y-1.5"><span className="text-sm font-medium text-slate-800">Reason</span><select value={reason} onChange={(event) => setReason(event.target.value as typeof reason)} className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100">{reasons.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="block space-y-1.5"><span className="text-sm font-medium text-slate-800">Details <span className="font-normal text-slate-500">(optional)</span></span><Textarea value={details} onChange={(event) => setDetails(event.target.value)} placeholder="Share enough context for a moderator to assess the concern." maxLength={2000} rows={5} /></label><DialogFooter><Button type="button" variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button type="submit" disabled={report.isPending} className="bg-sky-600 text-white hover:bg-sky-700">{report.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Submitting</> : "Submit report"}</Button></DialogFooter></form></DialogContent></Dialog>;
}
