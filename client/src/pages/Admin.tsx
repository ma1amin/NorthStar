...
export default function Admin() {
  ...
  const reviewSource = trpc.moderation.reviewSource.useMutation({ onSuccess: () => { toast.success(t("reviewSource")); refetchSources(); } });
      ...
      <div className="container max-w-6xl">
        ...
        <Tabs value={selectedTab} onValueChange={setSelectedTab} className="mt-7 w-full">
          ...
          <TabsContent value="submissions" className="mt-5">
            {submissionsLoading ? <LoadingState /> : submissions?.length ? <div className="grid gap-4">{submissions.map((submission: any) => <Card key={submission.id} className="ns-hover-lift border-l-4 border-l-amber-500 bg-white/90 p-6 shadow-sm"><div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_180px]"><div><div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-semibold text-slate-950">{submission.title}</h2><p className="mt-1 break-all text-sm text-sky-700">{submission.url}</p></div><Badge className="bg-amber-100 text-amber-800 hover:bg-amber-100">{t("pending")}</Badge></div><p className="mt-4 text-sm leading-6 text-slate-600">{submission.description || "No description provided."}</p><p className="mt-3 text-xs text-slate-500">{new Date(submission.createdAt).toLocaleDateString()}</p></div><div className="flex flex-col gap-2"><Button onClick={() => approveSubmission.mutate({ submissionId: submission.id })} disabled={approveSubmission.isPending} className="bg-emerald-600 text-white hover:bg-emerald-700"><CheckCircle className="mr-2 h-4 w-4" />{t("approve")}</Button><Button onClick={() => rejectSubmission.mutate({ submissionId: submission.id })} disabled={rejectSubmission.isPending} variant="outline" className="border-red-200 text-red-600 hover:bg-red-50"><XCircle className="mr-2 h-4 w-4" />{t("reject")}</Button></div></div></Card>)}</div> : <QueueEmpty label={t("submissions")} />}
          </TabsContent>

          <TabsContent value="relationships" className="mt-5">
            {relationshipsLoading ? <LoadingState /> : relationships?.length ? <div className="grid gap-4">{relationships.map((relationship: any) => <Card key={relationship.id} className="ns-hover-lift border-l-4 border-l-violet-500 bg-white/90 p-6 shadow-sm"><div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_180px]"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-violet-700">{relationshipLabels[relationship.type] ?? relationship.type}</p><h2 className="mt-2 text-xl font-semibold text-slate-950">Resource #{relationship.sourceId} <span className="text-slate-400">→</span> Resource #{relationship.targetId}</h2><p className="mt-4 text-sm leading-6 text-slate-600">Review this relationship for meaningful, non-redundant, and defensible graph value before publishing it.</p></div><div className="flex flex-col gap-2"><Button onClick={() => approveRelationship.mutate({ id: relationship.id })} disabled={approveRelationship.isPending} className="bg-emerald-600 text-white hover:bg-emerald-700"><CheckCircle className="mr-2 h-4 w-4" />{t("approve")}</Button><Button onClick={() => rejectRelationship.mutate({ relationshipId: relationship.id })} disabled={rejectRelationship.isPending} variant="outline" className="border-red-200 text-red-600 hover:bg-red-50"><XCircle className="mr-2 h-4 w-4" />{t("reject")}</Button></div></div></Card>)}</div> : <QueueEmpty label={t("relationships")} />}
          </TabsContent>

          <TabsContent value="sources" className="mt-5">
            {sourcesLoading ? <LoadingState /> : pendingSources?.length ? <div className="grid gap-4">{pendingSources.map((source: any) => <Card key={source.id} className="ns-hover-lift border-l-4 border-l-sky-500 bg-white/90 p-6 shadow-sm"><div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_190px]"><div><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-xl font-semibold text-slate-950">{source.resourceTitle}</h2><a href={source.url} target="_blank" rel="noreferrer" className="mt-1 block break-all text-sm text-sky-700 hover:text-sky-900">{source.url}</a></div><Badge className="capitalize bg-sky-100 text-sky-800 hover:bg-sky-100">{source.sourceType}</Badge></div>{source.attribution && <p className="mt-4 text-sm text-slate-700"><span className="font-medium">{t("sourceAttribution")}:</span> {source.attribution}</p>}{source.licenseNote && <p className="mt-2 text-sm text-slate-600">{source.licenseNote}</p>}<p className="mt-3 text-xs text-slate-500">{t("reviewedBy")}: {source.contributorName || "Unknown"} · {new Date(source.capturedAt).toLocaleDateString()}</p></div><div className="flex flex-col gap-2"><Button onClick={() => reviewSource.mutate({ sourceId: source.id, status: "approved" })} disabled={reviewSource.isPending} className="bg-emerald-600 text-white hover:bg-emerald-700"><CheckCircle className="mr-2 h-4 w-4" />{t("approveSource")}</Button><Button onClick={() => reviewSource.mutate({ sourceId: source.id, status: "superseded" })} disabled={reviewSource.isPending} variant="outline"><RefreshCw className="mr-2 h-4 w-4" />{t("supersedeSource")}</Button><Button onClick={() => reviewSource.mutate({ sourceId: source.id, status: "rejected" })} disabled={reviewSource.isPending} variant="outline" className="border-red-200 text-red-600 hover:bg-red-50"><XCircle className="mr-2 h-4 w-4" />{t("rejectSource")}</Button></div></div></Card>)}</div> : <QueueEmpty label={t("noPendingSources")} />}
          </TabsContent>
