...
export default function CollectionDetail() {
  ...
  const remove = (resourceId: number) => {
    if (window.confirm(t("removeResourceConfirm"))) removeResource.mutate({ collectionId, resourceId });
  };
  const removeCollection = () => {
    if (window.confirm(t("deleteCollectionConfirm"))) deleteCollection.mutate({ collectionId });
  };
  ...
  if (!collection) return <div className="container py-16"><Card className="p-10 text-center"><h1 className="text-2xl font-semibold text-slate-950">{t("collectionNotFound")}</h1><p className="mt-2 text-slate-600">{t("collectionUnavailable")}</p><Button className="mt-5" onClick={() => setLocation("/collections")}><ArrowLeft className="mr-2 h-4 w-4" /> {t("backToCollections")}</Button></Card></div>;

  return (
    <div className="min-h-screen bg-slate-50 py-8 md:py-12">
      <div className="container max-w-5xl">
        <Button variant="ghost" className="mb-5 px-0 text-slate-600 hover:bg-transparent hover:text-sky-700" onClick={() => setLocation("/collections")}><ArrowLeft className="mr-2 h-4 w-4" /> {t("allCollections")}</Button>
        <Card className="overflow-hidden border-slate-200 bg-white shadow-sm"><div className="h-28 bg-gradient-to-r from-slate-900 via-sky-900 to-cyan-700" /><div className="px-5 pb-7 md:px-8"><div className="-mt-8 flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><div className="flex items-end gap-4"><div className="flex h-16 w-16 items-center justify-center rounded-2xl border-4 border-white bg-sky-100 text-sky-700 shadow-lg"><FolderOpen className="h-7 w-7" /></div><div className="pb-1"><div className="mb-2 flex flex-wrap items-center gap-2"><Badge className={collection.isPublic ? "bg-emerald-100 text-emerald-800 hover:bg-emerald-100" : "bg-slate-200 text-slate-700 hover:bg-slate-200"}>{collection.isPublic ? <><Globe2 className="mr-1 h-3 w-3" /> {t("public")}</> : <><LockKeyhole className="mr-1 h-3 w-3" /> {t("private")}</>}</Badge></div><h1 className="text-2xl font-semibold tracking-tight text-slate-950">{collection.name}</h1></div></div><div className="flex flex-col gap-2 sm:flex-row"><Button variant="outline" onClick={share} className="border-slate-300">{copied ? <><Check className="mr-2 h-4 w-4 text-emerald-600" /> {t("linkCopied")}</> : <><Copy className="mr-2 h-4 w-4" /> {t("copyShareLink")}</>}</Button>{isOwner && <Button variant="outline" onClick={startEditing}><Pencil className="mr-2 h-4 w-4" /> {t("edit")}</Button>}</div></div><p className="mt-5 max-w-3xl text-sm leading-6 text-slate-600">{collection.description || t("curatedResourceSet")}</p><p className="mt-3 text-xs text-slate-500">{resources?.length ?? 0} {t("resources")} · {t("updated")} {new Date(collection.updatedAt).toLocaleDateString()}</p></div></Card>

        {editing && isOwner && <Card className="mt-5 border-sky-200 bg-sky-50/70 p-5 md:p-7"><h2 className="text-lg font-semibold text-slate-950">{t("edit")} {t("collections")}</h2><form onSubmit={save} className="mt-5 grid gap-4 md:grid-cols-2"><div className="space-y-2"><Label htmlFor="edit-collection-name">{t("collectionName")}</Label><Input id="edit-collection-name" value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} required maxLength={255} /></div><div className="space-y-2"><Label>{t("visibility")}</Label><div className="flex gap-2"><Button type="button" variant={form.isPublic ? "default" : "outline"} onClick={() => setForm((current) => ({ ...current, isPublic: true }))} className={form.isPublic ? "bg-sky-600 text-white hover:bg-sky-700" : ""}><Globe2 className="mr-2 h-4 w-4" /> {t("public")}</Button><Button type="button" variant={!form.isPublic ? "default" : "outline"} onClick={() => setForm((current) => ({ ...current, isPublic: false }))} className={!form.isPublic ? "bg-slate-800 text-white hover:bg-slate-900" : ""}><LockKeyhole className="mr-2 h-4 w-4" /> {t("private")}</Button></div></div><div className="space-y-2 md:col-span-2"><Label htmlFor="edit-collection-description">{t("collectionDescription")}</Label><Textarea id="edit-collection-description" value={form.description} onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} maxLength={2000} rows={4} /></div><div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between md:col-span-2"><Button type="button" variant="ghost" className="justify-start px-0 text-red-600 hover:bg-transparent hover:text-red-700" onClick={removeCollection} disabled={deleteCollection.isPending}><Trash2 className="mr-2 h-4 w-4" /> {t("deleteCollection")}</Button><div className="flex gap-3"><Button type="button" variant="outline" onClick={() => setEditing(false)}>{t("cancel")}</Button><Button type="submit" disabled={updateCollection.isPending} className="bg-sky-600 text-white hover:bg-sky-700">{updateCollection.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> {t("saving")}</> : t("saveChanges")}</Button></div></div></form></Card>}

        <div className="mt-8"><div className="mb-4 flex items-end justify-between"><div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-sky-700">{t("knowledgeStack")}</p><h2 className="mt-1 text-xl font-semibold text-slate-950">{t("resourcesInCollection")}</h2></div></div>{resourcesLoading ? <div className="flex justify-center py-16"><Loader2 className="h-7 w-7 animate-spin text-sky-600" /></div> : resources && resources.length > 0 ? <div className="grid gap-4 md:grid-cols-2">{resources.map((resource: any) => <Card key={resource.id} className="border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="font-semibold text-slate-950">{resource.title}</h3><p className="mt-1 line-clamp-2 text-sm leading-6 text-slate-600">{resource.description || resource.url}</p><Badge variant="outline" className="mt-3 capitalize">{resource.pricing.replace("_", " ")}</Badge></div><Button variant="ghost" size="icon" aria-label={`${t("open")} ${resource.title}`} onClick={() => setLocation(`/resource/${resource.slug}`)}><ArrowUpRight className="h-4 w-4" /></Button></div>{isOwner && <Button variant="ghost" size="sm" className="mt-4 px-0 text-xs text-red-600 hover:bg-transparent hover:text-red-700" onClick={() => remove(resource.id)} disabled={removeResource.isPending}>{t("removeFromCollection")}</Button>}</Card>)}</div> : <Card className="p-10 text-center shadow-sm"><FolderOpen className="mx-auto mb-4 h-12 w-12 text-slate-300" /><h3 className="font-semibold text-slate-950">{t("collectionEmpty")}</h3><p className="mt-2 text-sm text-slate-600">{t("collectionEmptyDescription")}</p><Button className="mt-5 bg-sky-600 text-white hover:bg-sky-700" onClick={() => setLocation("/browse")}>{t("explore")} {t("resources")}</Button></Card>}</div>
      </div>
    </div>
  );
}
